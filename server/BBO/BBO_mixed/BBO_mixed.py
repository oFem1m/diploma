"""
Алгоритм биогеографической оптимизации со смешанной миграцией частичная иммиграция

Смешивание линейной и синусоидальной кривых миграции по рангам r = 1..N
  lambda_mix = alpha * lambda_linear + (1 - alpha) * lambda_sinus
где
  lambda_linear = r / (N + 1)
  lambda_sinus = 0.5 * (1 - cos(pi * r / N))
mu = 1 - lambda_mix
"""
...
class BBO_Mixed:
    ...
    def optimize(self) -> Dict[str, Any]:
        ...
        for gen in range(self.cfg.max_generations):
            ...
            lambdas, mus = self._mixed_migration_rates(self.cfg.pop_size, self.cfg.mix_alpha)
    ...
    @staticmethod
    def _mixed_migration_rates(N: int, alpha: float):

        r = np.arange(1, N + 1, dtype=float)
        lambda_linear = r / (N + 1.0)
        lambda_sinus = 0.5 * (1.0 - np.cos(np.pi * r / N))
        lambdas = alpha * lambda_linear + (1.0 - alpha) * lambda_sinus
        mus = 1.0 - lambdas
        return lambdas, mus
...
if __name__ == "__main__":
    ...
    algo = BBO_Mixed(cfg, objective=rastrigin)
    res = algo.optimize()
