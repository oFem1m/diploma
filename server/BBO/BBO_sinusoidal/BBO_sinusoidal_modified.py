...
class BBO_Sinusoidal_Modified:
    def __init__(self, config: BBOConfigSinModified, objective: Callable[[np.ndarray], float],
    ...
    def optimize(self) -> Dict[str, Any]:
        ...
        for gen in range(self.cfg.max_generations):
            ...
            X_new = self._migration_step_sinusoidal(X, fit)
    ...
    def _migration_step_sinusoidal(self, X: np.ndarray, fit: np.ndarray) -> np.ndarray:
        N, d = X.shape
        idx = np.argsort(fit)
        ...
        imm_prob = 0.5 * (1.0 - np.cos(np.pi * r))
...
if __name__ == "__main__":
    ...
    algo = BBO_Sinusoidal_Modified(cfg, objective=rastrigin)
    res = algo.optimize()
